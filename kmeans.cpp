...
public:
   ...
   vector<vector<int>> run(vector<Point> &all_points)
   {
      ...
      for (int i = 0; i < total_points; i++)
      {
         ump[all_points[i].getCluster() - 1].push_back(all_points[i].getID());
      }
...
int main(int argc, char **argv)
{
   ...
   auto agents = kmeans.run(all_points);
   for (int i = 0; i < agents.size(); i++)
   {
      cout << "Delivery agent (" << i << ") will deliver [" << agents[i].size() << "] parcels -> { ";
      for (auto it : agents[i])
      {
         cout << it << ", ";
      }
      cout << " }";
